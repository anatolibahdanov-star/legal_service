...
import { getPlans, getTotalPlans, createPlan } from '@/src/repositories/subscriptions/repo';
import { DBSubscriptionPlan } from '@/src/interfaces/db';

export const dynamic = 'force-dynamic';

const cmnMsg = 'API SUBSCRIPTIONS ';
...
export async function GET(request: NextRequest) {
    ...
    let plans: DBSubscriptionPlan[] | null = [];
    let total = 0;
    try {
        plans = await getPlans(page, limit, sort, filter);
        total = await getTotalPlans(filter);
    } catch (err) {
    ...
    if (plans === null) {
        logger.error(msg + 'getPlans returned null (DB read failure).');
        return NextResponse.json({ success: false, message: 'during getting data from DB.' }, { status: 500 });
    }

    const response = NextResponse.json(plans, { status: 200 });
    const header_str = (_range[0] ?? 0) + '-' + (_range[1] ?? 99) + '/' + total;
    response.headers.set('Content-Range', 'subscriptions ' + header_str);
